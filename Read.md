...
for i in range(len(words)):
    countMap[words[i]] = 1 + countMap.get(words[i], 0) 

# another way with Counter
countMap = Counter(words)  

# get key and value from hash map
for word, word_count in countMap.items():
```
...
1. invert BT
![](https://i.imgur.com/r1rnzDB.png)


2.check if balanced tree.

approach 1 Top-down recursion. 
DFS, we go down until we hit base case (if not root: return 0), and run left and right subtree, then we check if abs(leftH - rightH)>1 return is_balenced = False and return 0(cos founction return int), then we get the height of a node with ( 1(root) + max(left,right) ). eventualy we return is_balanced

In approach 1, we perform redundant calculations when computing 
height
height. In each call to 
height
height, we require that the subtree's heights also be computed. Therefore, when working top down we will compute the height of a subtree once for every parent. We can remove the redundancy by first recursing on the children of the current node and then using their computed height to determine whether the current node is balanced.
